function [m, filename, imaxf, imeanf, pixh, pixw, nf, backupname] = data_cat(path_name, file_base, file_fmt, Fsi, Fsi_new, ratio, ttype)
...
%   Jinghao Lu 01/16/2016
    ...
    if ~contains(file_fmt, 'mat')
        ...
        backupname = [path_name, file_base, '_frame_all_raw.mat'];
        ...
        if overwrite_flag
            ...
            % for natural order (Kyu added, 20200204)
            dirst_Natorder = struct2cell(dirst);
            dirst_Natorder = dirst_Natorder(1, :);
            dirst_Natorder = natsortfiles(dirst_Natorder);
            for i = 1:length(dirst)
                dirst(i).name = char(dirst_Natorder(i));
            end
            fields = {'folder', 'datenum', 'bytes', 'date', 'isdir'};
            dirst = rmfield(dirst, fields);
...
%                         stt = stt / scl;
        ...
        if overwrite_flag
            ...
            m = matfile(filename, 'writable', true); % m = matfile(filename); Kyu changed 20200224
            imaxf = zeros(pixh, pixw);
            for i = 1: nbatch
                tmp = m.frame_all(1: pixh, 1: pixw, idbatch(i): idbatch(i + 1) - 1);
                imaxf = max(cat(3, max(tmp, [], 3), imaxf), [], 3);
