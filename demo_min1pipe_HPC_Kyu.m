...
result_folder = regexp(result_path, filesep, 'split');
result_scan = fullfile(result_path, '*.mat');
result_filelist = dir(result_scan);


fname = fullfile(result_filelist(1).folder, result_filelist(1).name);
frawname = fullfile(result_filelist(2).folder, result_filelist(2).name);
fregname = fullfile(result_filelist(3).folder, result_filelist(3).name);
...
nbatch = batch_compute(nsize); % read Memory
