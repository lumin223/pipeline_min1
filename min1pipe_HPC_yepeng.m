...
%   Jinghao Lu 06/10/2016
    ...
    %% get dataset info %%
    [path_name, file_base, file_fmt] = data_info %data_info_HPC(file_name, path_name);20200206
    ...
    for i = 1: length(file_base)
        ...
        if overwrite_flag
            ...
            %% delete black & dimmed frame
            [m, nf, timestamp]=delete_frames_and_import_timestamp(path_name);
            % At the end we might use the "m" variable they produced in
            % line87 instead of the input frame_all.mat, but here I'll just
            % use uigetfile inside the function.
